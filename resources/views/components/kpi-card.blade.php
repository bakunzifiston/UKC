...
@php
    ...
    $icons = [
        ...
        'users' => '<path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.897 1.125 1.125 0 0 0-.36-2.11 6.75 6.75 0 0 0-2.761-.75 6.75 6.75 0 0 0-2.761.75 1.125 1.125 0 0 0-.36 2.11ZM9 19.128a9.38 9.38 0 0 1-2.625.372 9.337 9.337 0 0 1-4.121-.897 1.125 1.125 0 0 1 .36-2.11 6.75 6.75 0 0 1 2.761-.75 6.75 6.75 0 0 1 2.761.75 1.125 1.125 0 0 1 .36 2.11ZM12 12.75a3.75 3.75 0 1 0 0-7.5 3.75 3.75 0 0 0 0 7.5Z" />',
        ...
        'leaf' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 18.75a6 6 0 0 0 6-6c0-4.125-3-7.5-6-9.75-3 2.25-6 5.625-6 9.75a6 6 0 0 0 6 6Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M12 18.75V9" />',
        ...
        'map' => '<path stroke-linecap="round" stroke-linejoin="round" d="M9 6.75V15m6-6v8.25m.503-3.498 3.544-1.181a.75.75 0 0 0 .497-.712V5.647a.75.75 0 0 0-.497-.712l-3.544-1.181a.75.75 0 0 0-.47 0L9.97 5.137a.75.75 0 0 1-.47 0L5.956 3.956A.75.75 0 0 0 5.25 4.65v8.61c0 .31.2.586.497.712l3.544 1.181a.75.75 0 0 0 .47 0l3.544-1.181a.75.75 0 0 1 .47 0Z" />',
        'warning' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />',
        'scale' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 3v17.25m0 0c-1.472 0-2.882.265-4.185.75M12 20.25c1.472 0 2.882.265 4.185.75M18.75 4.97A48.416 48.416 0 0 0 12 4.5c-2.291 0-4.545.16-6.75.47m13.5 0c.247.10.0.2.3.47m-14.1-.47c.1-.157.198-.315.3-.47m0 0A48.11 48.11 0 0 1 12 3c2.291 0 4.545.16 6.75.47" />',
...
<div {{ $attributes->merge(['class' => 'ui-card p-4']) }}>
    <div class="flex items-center gap-3">
        <div @class(['flex h-11 w-11 shrink-0 items-center justify-center rounded-full', $toneClass])>
        ...
        <div class="min-w-0 flex-1">
            ...
            <p class="mt-0.5 truncate text-lg font-bold tracking-tight text-slate-900">{{ $value }}</p>
            @if ($trend !== null)
                @php
                    $positive = $trendPositive ?? (is_numeric($trend) ? (float) $trend >= 0 : true);
                @endphp
                <p @class([
                    'mt-0.5 text-[11px] font-medium',
                    'text-brand' => $positive,
                    'text-rose-500' => ! $positive,
                ])>
                    {{ $trend }}
                </p>
            @elseif ($description)
                <p class="mt-0.5 truncate text-[11px] leading-snug text-slate-400">{{ $description }}</p>
            @endif
        </div>
    </div>
</div>
